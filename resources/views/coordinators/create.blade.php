...
@section('pageHeading', 'Create Coordinator')
@section('pageSubheading', 'Add a new program coordinator.')

@section('content')
    <div class="section-card">
        <div class="section-header">
            <h1 class="page-heading" style="margin:0;">New Coordinator</h1>
            <a href="{{ route('coordinators.index') }}" class="btn btn-secondary">Back to Coordinators</a>
        </div>

        <form method="POST" action="{{ route('coordinators.store') }}" style="padding-top:1rem;">
            @csrf
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1.25rem; margin-bottom:1.25rem;">
                <div style="display:flex; flex-direction:column; gap:0.3rem; grid-column:1/-1;">
                    ...
                    <input type="text" name="full_name" value="{{ old('full_name') }}" class="search-input" required placeholder="e.g. Dr. Juan Dela Cruz" />
                ...
                <div style="display:flex; flex-direction:column; gap:0.3rem;">
                    ...
                    <input type="text" name="position" value="{{ old('position') }}" class="search-input" placeholder="e.g. Program Manager" />
                ...
                <div style="display:flex; flex-direction:column; gap:0.3rem;">
                    ...
                    <input type="text" name="contact_number" value="{{ old('contact_number') }}" class="search-input" placeholder="e.g. 09171234567" />
                ...
                <div style="display:flex; flex-direction:column; gap:0.3rem;">
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" class="search-input" placeholder="e.g. juan@example.com" />
                ...
                <div style="display:flex; flex-direction:column; gap:0.3rem;">
                    ...
                    <select name="programs[]" class="search-input" multiple style="min-height:120px;">
                        @foreach($programs as $program)
                            <option value="{{ $program->id }}" @selected(in_array($program->id, old('programs', [])))>{{ $program->name }}</option>
                        @endforeach
                    </select>
