    ...
    public function index(Request $request)
    {
        ...
        if ($user) {
            ...
            if ($programIds->isNotEmpty()) {
                $userProgramNames = Program::whereIn('id', $programIds)
                    ->pluck('name')
                    ->unique()
                    ->values()
                    ->toArray();

                if (! empty($userProgramNames)) {
                    $userProgramScoped = true;

                    if ($program && in_array($program, $userProgramNames)) {
                        $programNames = [$program];
                    } else {
                        $programNames = $userProgramNames;
                        $program = '';
                    }
        ...
        if (! empty($programNames)) {
            $supplierStats->whereIn('items.stock_keeping_unit', $programNames);
        }

        $supplierStats = $supplierStats->select(
        ...
        return view('items.index', compact('items', 'search', 'status', 'category', 'categories', 'supplierStats', 'programs', 'program', 'userProgramNames', 'userProgramScoped'));
    }
    ...
    public function show(Item $item)
    {
        ...
        if ($user && $user->program_id) {
            ...
            if (! empty($programIds)) {
                ...
                if (! empty($programNames)) {
                    $item = $item->whereIn('stock_keeping_unit', $programNames)->firstOrFail();
                }
            }
        }
        ...
        if ($user) {
            ...
            if ($programIds->isNotEmpty()) {
                ...
                if (! empty($programNames)) {
                    ...
                    if ($program && in_array($program, $programNames)) {
                        $query->where('stock_keeping_unit', $program);
                    } else {
                        $query->whereIn('stock_keeping_unit', $programNames);
                        $program = '';
